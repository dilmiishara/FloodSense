  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        {[0.25, 0.5, 0.75, 1].map((f, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={padL + 2} y={y - 2} fontSize="7" fill="#ccc" fontFamily="DM Mono">
        ...
        <line ref={lineRef} x1="0" y1={padT} x2="0" y2={H}
          stroke={color} strokeWidth="1" strokeDasharray="3,3"
          opacity=".6" style={{ display: "none" }} />
        <circle ref={dotRef} cx="0" cy="0" r="4" fill={color}
          stroke="#fff" strokeWidth="1.5" style={{ display: "none" }} />
      </svg>
...
const WaterLevelChart = ({ C }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    const hourLabel =
      nearest[0] === 0 ? "Now" :
      nearest[0] > 0   ? `+${nearest[0]}H` : `${nearest[0]}H`;
    ...
    tip.innerHTML = `
      ...
      ${prob !== null ? `
        <div style="font-size:11px;color:${C.orange};font-weight:700;margin-top:3px">
          Probability: ${prob}%
        </div>` : ""}
    ...
    <div style={{ position: "relative" }} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`}
        ...
        {[3.8, 4.2, 4.6, 5.0, 5.4, 5.8, 6.2].map((v, i) => (
          <g key={i}>
            <line x1={padL} y1={ty(v)} x2={W - padR} y2={ty(v)} stroke="#f0f0f0" strokeWidth="1" />
            <text x={padL - 4} y={ty(v) + 3} fontSize="8" fill="#bbb"
              textAnchor="end" fontFamily="DM Mono">{v}m</text>
          </g>
        ))}
        <line x1={padL} y1={threshY} x2={W - padR} y2={threshY}
          stroke={C.red} strokeWidth="1.2" strokeDasharray="5,4" opacity=".5" />
        <text x={W - padR + 2} y={threshY + 3} fontSize="8" fill={C.red} fontWeight="700">5.2m</text>
        <path d={probPathD()} fill={C.orange} opacity=".07" />
        <polyline points={probPts.map((p) => `${tx(p[0])},${probY(p[3])}`).join(" ")}
          fill="none" stroke={C.orange} strokeWidth="1.5" strokeDasharray="4,3"
          opacity=".5" strokeLinecap="round" strokeLinejoin="round" />
        <path d={predPathD()} fill="url(#predGrad)" />
        <polyline points={toPolyline(actualPts, 0, 1)} fill="none"
          stroke="#1a1a2e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <polyline points={toPolyline(predPts, 0, 2)} fill="none"
          stroke={C.red} strokeWidth="2.5" strokeDasharray="8,4"
          strokeLinecap="round" strokeLinejoin="round" />
        {actualPts.map((p, i) => (
          <circle key={"a" + i} cx={tx(p[0])} cy={ty(p[1])} r="3" fill="#1a1a2e" opacity=".35" />
        ))}
        {predPts.filter((p) => p[0] !== 0).map((p, i) => (
          <circle key={"b" + i} cx={tx(p[0])} cy={ty(p[2])} r="3" fill={C.red} opacity=".35" />
        ))}
        <line x1={nowX} y1={padT - 6} x2={nowX} y2={padT + chartH}
          stroke="#555" strokeWidth="1.5" strokeDasharray="4,3" opacity=".4" />
        <rect x={nowX - 22} y={padT - 14} width="44" height="13" rx="4" fill="#1a1a2e" />
        <text x={nowX} y={padT - 4} fontSize="8" fill="#fff" textAnchor="middle" fontWeight="700">NOW</text>
        <circle cx={nowX} cy={ty(4.82)} r="5" fill="#1a1a2e" />
        <circle cx={nowX} cy={ty(4.82)} r="2.5" fill="#fff" />
        <circle cx={tx(9)} cy={ty(5.6)} r="4" fill={C.red} />
        <rect x={tx(9) - 30} y={ty(5.6) - 16} width="60" height="13" rx="3" fill={C.red} />
        <text x={tx(9)} y={ty(5.6) - 6} fontSize="8" fill="#fff" textAnchor="middle" fontWeight="700">
          PEAK 5.6m
        </text>
        {[[-6,"-6H"],[-3,"-3H"],[0,"Now"],[3,"+3H"],[6,"+6H"],[9,"+9H"],[12,"+12H"],[18,"+18H"],[24,"+24H"]].map(([h,l],i) => (
          <text key={i} x={tx(h)} y={H - 4} fontSize="8"
            fill={h === 0 ? "#333" : "#bbb"} textAnchor="middle"
            fontFamily="DM Mono" fontWeight={h === 0 ? "700" : "400"}>{l}</text>
        ))}
      </svg>
...
const StatCard = ({ label, value, valColor, sub, subColor, dark, extra }) => (
  <Card style={{ padding: "16px 18px", background: dark ? C.dark : C.white, color: dark ? "#fff" : C.dark }}>
    <div style={{ fontSize:11, fontWeight:600, textTransform:"uppercase",
      letterSpacing:0.4, color: dark ? "#888" : "#aaa", marginBottom:8 }}>{label}</div>
    <div style={{ fontSize:30, fontWeight:900, letterSpacing:-1, lineHeight:1,
      color: valColor || (dark ? "#fff" : C.dark) }}>{value}</div>
    <div style={{ fontSize:11, color: subColor || (dark ? "#aaa" : C.mid),
      marginTop:5, fontWeight:600 }}>{sub}</div>
    {extra}
  </Card>
);
        ...
        <div style={{ display:"flex", margin:"12px 14px 14px" }}>
          ...
          <div style={{
            ...
            <div className="fadeUp" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
              <div>
                <div style={{ fontSize:20, fontWeight:900, letterSpacing:-0.4 }}>Prediction</div>
                      ...
                      <Card key={id} style={{ padding:"16px 18px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"flex-start",
                          ...
                          <Badge type={
                            loading ? "safe" :
                            status === "normal"   ? "safe"     :
                            status === "warning"  ? "medium"   : "critical"
                          }>
                        ...
                        {loading && (
                          <div style={{
                            height:100, background:"#f4f4f4", borderRadius:8,
                        ...
                        {!loading && !error && (
                          <>
                            <div style={{ margin:"10px 0 12px" }}>
                              <span style={{ fontSize:38, fontWeight:900, color:C.dark,
                            ...
                            <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
                              {[times[0], times[7], times[15], times[23]].map((l, i) => (
                                <span key={i} style={{ fontSize:9, color:"#ccc", fontFamily:"DM Mono" }}>
                                  {l}
                                </span>
                              ))}
                            </div>
                ...
                <Card className="fadeUp">
                  ...
                  <svg viewBox="0 0 720 175" style={{ width:"100%", overflow:"visible" }}>
                    {[20,52,84,116,148].map((y) => (
                      <line key={y} x1="60" y1={y} x2="700" y2={y} stroke="#f0f0f0" strokeWidth="1" />
                    ))}
                    {["6.0m","5.5m","5.0m","4.5m","4.0m"].map((l, i) => (
                      <text key={i} x="52" y={24 + i * 32} fontSize="9" fill="#bbb"
                        textAnchor="end" fontFamily="DM Mono">{l}</text>
                    ))}
                    <line x1="60" y1="40" x2="700" y2="40" stroke={C.red}
                      strokeWidth="1.5" strokeDasharray="6,4" opacity=".5" />
                    <text x="702" y="43" fontSize="9" fill={C.red} fontWeight="700">5.2m</text>
                    <path d="M390,100 L460,82 L530,60 L600,36 L670,18 L670,32 L600,52 L530,76 L460,96 L390,112 Z"
                      fill={C.red} opacity=".07" />
                    <polyline points="60,148 130,140 200,132 270,122 340,110 390,100"
                      fill="none" stroke={C.dark} strokeWidth="2.5"
                      strokeLinecap="round" strokeLinejoin="round" />
                    <polyline points="390,100 460,82 530,60 600,36 670,18"
                      fill="none" stroke={C.red} strokeWidth="2.5" strokeDasharray="8,4"
                      strokeLinecap="round" strokeLinejoin="round" />
                    <line x1="390" y1="12" x2="390" y2="160" stroke="#555"
                      strokeWidth="1.5" strokeDasharray="4,3" opacity=".4" />
                    ...
                    {[["60","−3H","#bbb"],["200","−2H","#bbb"],["340","−1H","#bbb"],
                      ["390","14:32","#333"],["460","+1H",C.red],["530","+2H",C.red],
                      ["600","+4H",C.red],["670","+6H",C.red]].map(([x,l,c],i) => (
                      <text key={i} x={x} y="170" fontSize="9" fill={c}
                        textAnchor="middle" fontFamily="DM Mono">{l}</text>
                    ))}
                ...
                <div className="fadeUp" style={{ display:"flex", gap:12, alignItems:"stretch" }}>
                  <Card style={{ flex:1, padding:"16px 18px", display:"flex", flexDirection:"column" }}>
                    <div style={{ display:"flex", alignItems:"flex-start",
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:10,
                        fontWeight:700, color:C.green, background:"#e5f8ee",
                        padding:"4px 10px", borderRadius:20 }}>
                        <div className="blink" style={{ width:6, height:6,
                          borderRadius:"50%", background:C.green }} />
                        LIVE
                      </div>
                    </div>
                    <canvas id="riverCanvas" style={{ width:"100%", height:80, display:"block" }} />
                    <div style={{ display:"flex", gap:20, marginTop:10, paddingTop:10,
                      ...
                      {[
                        ...
                        <div key={i} style={{ display:"flex", flexDirection:"column", gap:2 }}>
                          <span style={{ fontSize:9, fontWeight:600, textTransform:"uppercase",
                            letterSpacing:0.4, color:"#aaa" }}>{lbl}</span>
                          <span id={id || undefined} style={{ fontSize:13, fontWeight:800,
                            fontFamily:"DM Mono", color:c }}>{val}</span>
                        </div>
                      ))}
                    </div>
                  </Card>
                  ...
                  <div style={{ width:400, background:C.dark, borderRadius:14, padding:18,
                    ...
                    <div style={{ fontSize:11, fontWeight:700, textTransform:"uppercase",
                      letterSpacing:0.5, color:"#888", marginBottom:14 }}>
                      ⚡ Recommended Actions
                    </div>
                    {[
                      ...
                      <div key={i} style={{ display:"flex", gap:10, marginBottom:12 }}>
                        <div>
                          <div style={{ fontSize:12, fontWeight:700 }}>{head}</div>
                          <div style={{ fontSize:10, color:"#bbb", marginTop:3, lineHeight:1.5 }}>{body}</div>
                        </div>
                      </div>
                    ))}
                    <button style={{ marginTop:"auto", width:"100%", padding:10, background:C.red,
            ...
            {tab === "rainfall" && (
              <>
                <div className="fadeUp" style={{ display:"grid",
                ...
                <Card className="fadeUp">
                    ...
                    {[[10,false],[13,false],[16,false],[18,false],[14,false],[12,false],
                      ...
                      <div key={i} style={{ flex:1, display:"flex", flexDirection:"column",
                        ...
                        <div style={{ width:"100%", height:`${(h / 40) * 100}%`, minHeight:4,
                          background: pred ? "#9ec4ee" : "#4a8ee0", borderRadius:"3px 3px 0 0",
                  ...
                  <div style={{ display:"flex", gap:5, marginTop:4 }}>
                    {["08:00","09:00","10:00","11:00","12:00","14:00",
                      "15:00","16:00","17:00","18:00","20:00","02:00"].map((l, i) => (
                      <div key={i} style={{ flex:1, fontSize:8,
                        color: i >= 6 ? C.blue : "#bbb", textAlign:"center",
                        fontFamily:"DM Mono", fontWeight: i === 7 ? "700" : "400" }}>{l}</div>
                    ))}
                  </div>
                </Card>
          ...
          <div style={{ fontSize:11, color:C.mid, marginTop:2 }}>
            ...
            {tab === "heatmap" && (
              <div className="fadeUp" style={{ display:"flex", gap:12 }}>
                ...
                <Card style={{ width:280, display:"flex", flexDirection:"column" }}>
                  <div style={{ fontSize:14, fontWeight:700, marginBottom:4 }}>District Risk Rankings</div>
                  <div style={{ fontSize:11, color:"#aaa", marginBottom:10 }}>
                  ...
                  <div style={{ fontSize:9, color:"#ccc", textAlign:"center",
                    marginTop:8, letterSpacing:0.3 }}>↕ scroll to see all districts</div>
                </Card>
              </div>
            )}
            ...
            {tab === "floodpred" && (
              <>
                <div className="fadeUp" style={{ display:"grid",
                  gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
                  <StatCard label="Flood Probability (6H)" value="78%" valColor={C.red}
                    sub="▲ High risk — rising fast" subColor={C.red}
                    extra={<div style={{ marginTop:8 }}>
                      <div style={{ background:"#f0f0f0", height:7, borderRadius:4, overflow:"hidden" }}>
                        <div style={{ width:"78%", background:`linear-gradient(90deg,${C.orange},${C.red})`,
                          height:"100%", borderRadius:4 }} />
                      </div>
                    </div>} />
                  <StatCard label="Predicted Peak Level" value="5.6m" valColor={C.red}
                    sub="▲ Exceeds 5.2m threshold" subColor={C.red}
                    extra={<div style={{ marginTop:8, display:"flex", alignItems:"center", gap:6 }}>
                      <div style={{ flex:1, background:"#f0f0f0", height:7, borderRadius:4, overflow:"hidden" }}>
                        <div style={{ width:"92%", background:C.red, height:"100%", borderRadius:4 }} />
                      </div>
                      <span style={{ fontSize:10, color:C.red, fontWeight:700 }}>92%</span>
                    </div>} />
                  <StatCard label="Expected Flood Onset" value="~3 hrs" valColor={C.orange}
                    sub="17:00–17:30 LKT window" />
                  <StatCard label="Affected Population" value="12,400" valColor={C.dark}
                ...
                <div className="fadeUp" style={{ display:"flex", gap:12, alignItems:"stretch" }}>
                  <Card style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", gap:16, marginBottom:12 }}>
                      {[["#1a1a2e","Actual Level","solid"],[C.red,"Predicted Level","dashed"],
                        [C.orange,"Flood Probability","dashed"]].map(([c,l,s],i) => (
                  ...
                  <Card style={{ width:280, display:"flex", flexDirection:"column" }}>
                    <div style={{ fontSize:14, fontWeight:700, marginBottom:4 }}>District Flood Risk</div>
                    <div style={{ fontSize:11, color:"#aaa", marginBottom:10 }}>
                      Ratnapura District · All DSD areas
                    </div>
                    <div style={{ overflowY:"auto", flex:1, maxHeight:280, paddingRight:2 }}>
                    ...
                    <div style={{ fontSize:9, color:"#ccc", textAlign:"center",
                      marginTop:8, letterSpacing:0.3 }}>↕ scroll to see all divisions</div>
                  </Card>
                </div>
              </>
            )}
