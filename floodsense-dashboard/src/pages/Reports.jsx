            ...
            {tab === "generate" && (
                <div className="fadeUp" style={{ display: "grid", gridTemplateColumns: "1.4fr 0.6fr", gap: 16, alignItems: 'stretch' }}>
                  <Card style={{ padding: 28, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 24, color: "var(--text)", display: 'flex', alignItems: 'center', gap: 8 }}>
                        Configuration
                      </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: 24 }}>
                      ...
                      <Btn variant="primary" onClick={handleGenerate} disabled={loading} style={{ padding: '12px 32px' }}>
                  ...
                  {/* QUICK GUIDE */}
                  <Card style={{ background: "var(--surface-alt)", border: "1.5px dashed var(--border-mid)", padding: 28 }}>
                    <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 20, color: "var(--text)", display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Info size={18} color="var(--primary)" /> Quick Guide
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                      <div style={guideItem}>
                        <div style={guideIcon}><ShieldCheck size={14} color="var(--green)" /></div>
                        <p style={guideText}><b style={{ color: "var(--text)" }}>Select Region:</b> Use specific region for localized sensor data comparison.</p>
                      </div>
                      <div style={guideItem}>
                        <div style={guideIcon}><Clock size={14} color="var(--primary)" /></div>
                        <p style={guideText}><b style={{ color: "var(--text)" }}>Timeframe:</b> Analytical accuracy depends on the selected date range logs.</p>
                      </div>
                      <div style={guideItem}>
                        <div style={guideIcon}><Search size={14} color="var(--text-muted)" /></div>
                        <p style={guideText}><b style={{ color: "var(--text)" }}>Tracking:</b> Once generated, use the Archive tab to track historical files.</p>
                      </div>
                    </div>
                  </Card>
                </div>
            ...
            {tab === "archive" && (
                <div className="fadeUp">
                  <div style={{ marginBottom: 18, position: 'relative', maxWidth: 400 }}>
                    <Search size={16} color="var(--text-muted)" style={{ position: 'absolute', left: 14, top: 14, pointerEvents: "none" }} />
                    <input
                        type="text"
                        placeholder="Search by name, region or date..."
                        value={searchTerm}
                  ...
                  <Card style={{ padding: 0, overflow: 'hidden' }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      {filteredReports.length > 0 ? filteredReports.map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px solid var(--border)", transition: '0.2s' }}>
                            <td style={{ padding: "16px 20px" }}>
                              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{r.name}</div>
                              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3, fontWeight: 600 }}>{r.type.toUpperCase()}</div>
                            </td>
                            <td style={{ padding: "16px 20px", fontSize: 13, color: "var(--text-mid)" }}>{r.area?.name || "Global"}</td>
                            <td style={{ padding: "16px 20px", fontSize: 13, color: "var(--text-muted)", fontFamily: "'DM Mono', monospace" }}>
                              {new Date(r.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                            </td>
                            <td style={{ padding: "16px 20px", display: 'flex', justifyContent: 'center', gap: 12 }}>
                              <button style={actionBtn("var(--green-bg)", "var(--green)")} onClick={() => handleDownload(r.file_path, r.name)} title="Download"><Download size={16} /></button>
                              <button style={actionBtn("var(--red-bg)", "var(--red)")} onClick={() => { setSelectedReport(r); setIsModalOpen(true); }} title="Delete"><Trash2 size={16} /></button>
                            </td>
                          </tr>
                      )) : (
                          <tr><td colSpan="4" style={{ padding: 60, textAlign: 'center', color: "var(--text-muted)", fontStyle: 'italic' }}>No matching reports found in archive.</td></tr>
                      )}
                      </tbody>
                    </table>
                  </Card>
...
// ── Style constants — CSS variables only, works in light / dark / contrast ─────
const thStyle = { textAlign: "left", padding: "18px 20px", fontSize: 11, color: "var(--text-muted)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.8px" };
const statCard = (color) => ({ padding: "22px", borderLeft: `5px solid ${color}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' });
const statLabel = { fontSize: 11, fontWeight: 800, color: "var(--text-muted)", letterSpacing: 0.5 };
const statVal   = { fontSize: 28, fontWeight: 900, marginTop: 6, color: "var(--text)" };

const searchField = {
  width: '100%', padding: '12px 16px 12px 42px', borderRadius: 14,
  border: "1.5px solid var(--border)", fontSize: 14, outline: 'none', transition: '0.3s',
  background: "var(--surface-alt)", color: "var(--text)", fontFamily: "inherit",
};
...
const actionBtn = (bg, color) => ({
  padding: "10px", borderRadius: 12, background: bg, color,
  border: "none", cursor: "pointer", display: 'flex', alignItems: 'center', transition: '0.2s',
});

const guideItem = { display: 'flex', gap: 12, alignItems: 'flex-start' };
const guideIcon = { padding: 8, background: "var(--surface)", borderRadius: 10, boxShadow: "var(--shadow)", display: 'flex', marginTop: 2 };
const guideText = { fontSize: 12, color: "var(--text-mid)", margin: 0, lineHeight: '1.6' };

const modalOverlay = {
  position: "fixed", top: 0, left: 0, width: "100%", height: "100%",
  background: "rgba(15,27,61,0.5)", display: "flex", alignItems: "center",
...
const cancelBtn = {
  ...
  color: "var(--text)", fontWeight: 800, cursor: "pointer", fontSize: 13,
};
const confirmBtn = {
  flex: 1, padding: "14px", borderRadius: 12, border: "none",
  background: "var(--red)", color: "#fff", fontWeight: 800, cursor: "pointer", fontSize: 13,
};
